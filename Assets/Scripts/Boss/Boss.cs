...
public class Boss : MonoBehaviour
{
    ...
    IEnumerator FirstAttack()
    {
        ...
        {
            GameObject summonedObj = Instantiate(enemySummoner, _firstAttackPositions[i].transform.position, Quaternion.identity);

            if (summonedObj.transform.position.y > ceiling.transform.position.y)
            {
                float newYPos = ceiling.position.y - (summonedObj.transform.localScale.y);
                summonedObj.transform.position = new Vector3(summonedObj.transform.position.x, newYPos, summonedObj.transform.position.z);
            }
