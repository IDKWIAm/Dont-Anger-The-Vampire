...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] CapsuleCollider2D vampireColl;
    [SerializeField] CircleCollider2D batColl;
    ...
    private void MoveUpdate()
    {
        ...
        if (xAxis != 0)
        {
            _playerRigitbody.velocity = new Vector2(speed * xAxis, _playerRigitbody.velocity.y);
            _isMovingX = true;

            if (_faceRight && xAxis < 0)
            {
                MirrorCharacter();
                _faceRight = false;
            }
            else if (!_faceRight && xAxis > 0)
            {
                MirrorCharacter();
                _faceRight = true;
            }

            if (IsGrounded() && walkingSound.isPlaying == false)
                walkingSound.Play();
        }

        if (xAxis == 0 || !IsGrounded() || _isFlying || _dashLeft || _dashRight)
            walkingSound.Stop();
        ...
        if (_isFlying)
        {
            ...
            if (yAxis != 0)
            {
                _playerRigitbody.velocity = new Vector2(_playerRigitbody.velocity.x, speed * yAxis);
                _isMovingY = true;
            }
    ...
    private void ChangeFormUpdate()
    {
        ...
        if (Input.GetButtonDown("Fly") && !_flyAbilityPerformed)
        {
            ...
            _playerRigitbody.velocity = new Vector2(_playerRigitbody.velocity.x, 0);
            _isFlying = true;
            vampireColl.enabled = false;
            batColl.enabled = true;
        }
        if (Input.GetButtonUp("Fly") || _flyingTimer <= 0 && _isFlying)
        {
            ...
            vampireForm.layer = 11;
            vampireColl.enabled = true;
            batColl.enabled = false;
