...
{
    [SerializeField] bool forceSend;
    [SerializeField] bool freezeSending;
    [SerializeField] bool sendOnContact;
    [SerializeField] bool sendOnce;
    [SerializeField] float letterAppearingDelay = 0.03f;
    ...
    private void Update()
    {
        if (Input.GetButtonDown("Interact") && _collided && !dialogueManager.IsRunning() && !freezeSending)
        {
            SendMessage();
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.tag == "Player")
        {
            if (sendOnContact)
            {
                if (forceSend) dialogueManager.CloseWindow();
                SendMessage();
            }
            else _collided = true;
        }
